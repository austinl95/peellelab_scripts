function [t,F,p,df,R2,cR2,B] = glm(y,X,c,pflag);
...
if size(c,2)==1
  ...
  cR2 = F2R(F,1,df);
  R2 = 1 - (r'*r) / (y'*y); 
  fprintf('T(%d)=%f, p(one-tailed)=%f  (R2=%3.2f; overall R2=%3.2f)\n',df,t,p,cR2,R2);
else
  ...
  cR2 = F2R(F,df(1),df(2));
  R2 = 1 - (r'*r) / (y'*y); 
  fprintf('F(%d,%d)=%f, p(two-tailed)=%f  (R2=%3.2f; overall R2=%3.2f)\n',df(1),df(2),F,p,cR2,R2);
end
