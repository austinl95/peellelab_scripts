...
%
%	usage: aa_jpeg_crawler(aap, exclude_list)
%
% INPUT
%
%	aap - aap struct of analysis to crawl
%
%   exclude_list - cell array of strings used to ID files to be
%       excluded from results (e.g. {"tsdiffana"} will exclude all
%       jpegs from aamod_tsdiffana_* - sometimes volumes mounted
%       across sshfs don't play nice with large filelists.)
%
% Usage
%
...
% 07/24 -- make sections collapsible
%
...
if (isempty(aap))
	disp('Usage: aa_jpeg_crawler(aap)');
	return;
end

% if no exclusion list, create one with a nonsensical string
...
command = sprintf('find -s `pwd` -name \\*.jpg | sort -t/ -k%d > montage_temp01.txt', root_depth+4);
status = system(command);

if (status); cleanup_and_exit(1); end

% apply the exclude_list
% note this makes a full TOC, but the excluded sections will be empty

command = sprintf('-e %s ', exclude_list{:});
command = [ 'grep -v ' command 'montage_temp01.txt > montage_temp.txt'];
status = system(command);
